...
### 1. Offline Suggestions & Optimization (Offline Suggest)
The **Offline Suggest** feature in Getty Resolver leverages a local semantic AI model (such as fast MiniLM or more accurate BGE) combined with a pre-generated vector database of approved Getty terms to optimize your metadata entirely offline. When executing `run_offline_suggest`, the process runs through three key phases. First, it analyzes known, approved keywords from your selected images and converts them via a local ONNX model into a unified contextual vector ($context\_vec$) that represents the overall theme of the photos. Second, it cleans existing keywords by evaluating their status: unrecognized words are automatically replaced with the closest matching Getty term if their combined relevance score reaches the strict threshold of $\ge 0.70$ (otherwise, they are discarded as typos); ambiguous terms with multiple meanings are automatically mapped to the definition that best fits the image context (provided it meets the UI threshold); and verified terms remain securely mapped. Finally, the system initiates semantic expansion by suggesting new, highly relevant Getty terms that are not yet present. The maximum number of new suggestions is capped by the *New AI terms* setting in the UI, which sorts candidates by semantic proximity, while the *Suggest Threshold* acts as a quality filter to ensure only highly accurate terms enter your metadata table.

### 2. User Getty Dictionary & Personalization Blacklist
