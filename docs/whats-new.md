...
* **[Intelligent Semantic Disambiguation Assistant](/docs/getty-images-esp-metadata-optimizer.html#xoffline-resolving-local-semantics):** The Cloud AI engine can study the visual context of your photo and automatically decide which precise meaning of a word to select (e.g., reliably distinguishing a 'crane' bird from a 'crane' construction machine).
* **[Persistent User Dictionary](/docs/getty-images-esp-metadata-optimizer.html#1-built-in-master-dictionary--non-destructive-workflow):** The system features personal memory. Once you manually map a missing keyword or a custom name to an existing term from the Master database, the application remembers it for all future exports.
