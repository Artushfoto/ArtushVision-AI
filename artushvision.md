...
-   {user_hint} - Hint entered by the user in the main window
-   {filename} - Filename (without extension)
-   {folder_context} - Folder name (for context)
-   {date_info} - Capture date (DD.MM.YYYY)
-   {existing_keywords} - Existing keywords loaded from the photo
-   {existing_title} - Existing title loaded from the photo
-   {existing_description} - Existing description loaded from the photo
-   {existing_rating} - Photo rating (1-5 stars)

### Geolocation:

1. Automatically generated by ArtushVision from the photo's GPS coordinates:

-   {gps_raw} - GPS coordinates (e.g. 50.08, 14.43)
-   {maps_link} - Link to Google Maps
-   {loc_hint} - Location from reverse geocoding (City, State)
-   {city} and {country} - City and Country separately  
    Example: "Generate title in the format: [What is in the photo] in {city}, {country}."

2. Loaded from text IPTC/XMP metadata (e.g., entered manually, automatically applied during Lightroom import, or written by digiKam):

-   {existing_location} - Location
-   {existing_sublocation} - Sub-location
-   {existing_city} - City
-   {existing_state} - State/Province
-   {existing_country} - Country
-   {existing_country_code} - ISO Country Code

### Technical and EXIF parameters (For better AI tagging):

-   {camera_model} - Camera or Drone model  
    From EXIF (e.g., "DJI Mavic 3", "GoPro HERO11"). Helps AI understand context (e.g., you can command it: for drones, add tags drone photography, aerial view; for GoPro, tag action camera).
-   {exposure_info} - Exposure parameters  
    Time, aperture, and ISO (e.g., "1/2s, f/1.8, ISO 3200"). Helps AI infer tags (e.g., long shutter -> long exposure, motion blur; low aperture -> bokeh).
-   {aspect_ratio} - Orientation and aspect ratio  
    E.g., "Landscape", "Portrait", "Panorama", "Square". Helps AI compensate for format loss when the photo is resized, to generate tags like: panoramic, vertical, copy space.
-   {flash_used} - Flash usage  
    Yes/No. Significantly helps AI in generating light-related tags (e.g., flash photography, studio lighting).
-   {lens_hint} - Lens information (from EXIF)
-   {allowed_categories} - List of categories from the Category Matrix (e.g. for AI to pick exactly 3 matching categories).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY1Njg1Njk3NSwtNzg1ODQ5OTQ2LDE4NT
